#!/usr/bin/env python
"""
Docker-compatible chat client using Ollama with OpenMCP tool support
"""
...
# Docker service URLs
OPENMCP_BASE = "http://localhost:8000"  # Docker mapped port
CALC_API_BASE = "http://localhost:8001"  # Docker mapped port
...
def check_services():
    ...
    # Check OpenMCP (Docker)
    try:
        r = requests.get(f"{OPENMCP_BASE}/health", timeout=2)
        if r.status_code == 200:
            console.print("✅ OpenMCP is running (Docker)", style="green")
        else:
            console.print("❌ OpenMCP is not responding properly", style="red")
            services_ok = False
    except:
        console.print("❌ OpenMCP is not running on port 8000", style="red")
        console.print("   Run: docker-compose up -d", style="yellow")
        services_ok = False
    
    # Check Calculator API (Docker)
    try:
        r = requests.get(f"{CALC_API_BASE}/health", timeout=2)
        if r.status_code == 200:
            console.print("✅ Calculator API is running (Docker)", style="green")
        else:
            console.print("❌ Calculator API is not responding properly", style="red")
            services_ok = False
    except:
        console.print("❌ Calculator API is not running on port 8001", style="red")
        console.print("   Run: docker-compose up -d", style="yellow")
        services_ok = False
    ...
    try:
        ...
        if r.status_code == 200:
            console.print("✅ Ollama is running (Local)", style="green")
        else:
...
def main():
    console.print(Panel.fit(
        "[bold blue]OpenMCP Docker Chat Client[/bold blue]\n"
        "Chat with AI using containerized services",
        border_style="blue"
    ))
    ...
    if not check_services():
        ...
        console.print("1. Run: docker-compose up -d")
        console.print("2. Install and run Ollama locally from https://ollama.ai")
        return
    
    # Register calculator API with OpenMCP (Docker version)
    console.print("\n[bold]Registering Calculator API...[/bold]")
    try:
        # Use Docker-specific spec that points to container network
        spec_data = {
            "spec_path": "./specs/calculator-api-docker.yaml"
        }
...
Be conversational and explain what you're doing when using tools."""
    ...
    ollama_client = OllamaIntegration(config, openmcp_base=OPENMCP_BASE)
